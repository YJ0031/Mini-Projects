...
def main(): 
    ...
    plaintext = "ThisisPythonProgramming".encode("utf-8") #Converting to byte stream
    #t = w.encrypt(0x6f7220676e696c63)

    
    #converting the string to hex format
    #t = (binascii.hexlify(plaintext))

    h_plaintext = plaintext.hex() #converting to hex

    i_plaintext = int(h_plaintext, 16) #converting to int
    
    enc_initial = time.time()
    encrypted = w.encrypt(i_plaintext)
    enc_final = time.time()

    print(f"Encrypted text: {encrypted}")

    dec_initial = time.time()
    ...
    text = hex(decrypted) #converting to hex


    text = bytes.fromhex(text[2:]) #converting to bytestream
    
    text = text.decode("utf-8") #Converting to string

    print(text)
    #----------------- 

    total_time = (dec_final - dec_initial) + (dec_final - dec_initial)
    print(f"Total execution time: {total_time}")

    #Memory usage
    print("CPU % used: ", psutil.cpu_percent())

    #print(bytearray.fromhex(p).decode())
